...
{
    VECTOR fixedTargetPosition = VGet(0.0f, 0.0f, 0.0f);

    float yawRadian = DEFAULT_YAW * (DX_PI_F / 180.0f);
    float pitchRadian = DEFAULT_VERTICAL * (DX_PI_F / 180.0f);

    VECTOR fixedCameraPosition;
    fixedCameraPosition.x = fixedTargetPosition.x + DEFAULT_DISTANCE * cosf(pitchRadian) * sinf(yawRadian);
    fixedCameraPosition.y = fixedTargetPosition.y + DEFAULT_DISTANCE * sinf(pitchRadian);
    fixedCameraPosition.z = fixedTargetPosition.z - DEFAULT_DISTANCE * cosf(pitchRadian) * cosf(yawRadian);

    if (shakeStep_ > 0.0f)
    {
        fixedCameraPosition = VAdd(fixedCameraPosition, shakeDir_);
        fixedTargetPosition = VAdd(fixedTargetPosition, shakeDir_);
    }

    SetCameraNearFar(CLIP_NEAR, CLIP_FAR);
    SetCameraPositionAndTarget_UpVecY(fixedCameraPosition, fixedTargetPosition);

    targetPos_ = fixedTargetPosition;
}
